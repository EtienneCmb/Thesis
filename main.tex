\documentclass[sommairechap,stylejchiquet]{these_gi}



\begin{document}

% ==================================================================
% OPTIONS D'AFFICHAGE
% non-définitif (soumis aux rapporteurs) ou  définitif
\definitiftrue
% \definitiffalse

% ==================================================================
% RENSEIGNEMENTS SUR LA THÈSE
\titleFR{Décodage des intentions et des exécutions motrices : Étude du rôle des oscillations cérébrales via l'apprentissage machine et développement d'outils open-source}
\titleEN{Decoding motor intentions and movement execution: Investigating the role of cerebral oscillations using machine learning and development of open-source tools}
\include{Resume/EN}
\author{Etienne Combrisson}
\address{e.combrisson@gmail.com}
\universite{université claude bernard lyon 1}
\laboratoire{}
\specialite{spécialité de la thèse}
\datesoutenance{09/2016}
\datesoumission{la date de soumission aux rapporteurs}
\jury{\begin{tabular}{llll}
    M\up{me} & \textsc{Erika Raté} & Université à la Menthe & (Rapporteur) \\
    M. & \textsc{Jacques Ouille} & Université à la Fraise & (Rapporteur) \\
    M. & \textsc{Henri Zoto} & Laboratoire laborieux & (Rapporteur) \\
    M. & \textsc{Jean File} & Indienne & (Directeur) \\
       & etc. &  \\
  \end{tabular}    
}
{
% ==================================================================
% DÉDICACE
\dedicate{À mes parents, Isabelle et Didier Prévot,\\ Merci}
 
% ==================================================================
% DEBUT DE LA PRÉFACE
\beforepreface
 
% ==================================================================
% COUVERTURE : RESUME ET MOTS-CLÉS
\abstractpage
 
% remerciements
\include{Remerc/remerc}
 
% table des matières générale
{
\hypersetup{linkcolor=black}
\tableofcontents
 
% affiche la liste des figures
\newpage
\listoffigures
}
% ==================================================================
\afterpreface

% ==================================================================
% NOTATIONS
\include{Nota/nota}

% ==================================================================
% AVANT-PROPOS
\setcounter{secnumdepth}{4}
\include{Intro/00_intro}
\adjustmtc

% ==================================================================
% CONTENU GÉNÉRAL
% \pagestyle{headings}
\include{Chap1/chap1}
\include{Chap2/chap2}
\include{Chap3/chap3}
\include{Chap4/chap4}
\include{Chap5/chap5}
\include{Chap6/chap6}

% ==================================================================
% CONCLUSION
% \makeatletter
% \renewcommand*{\toclevel@chapter}{-1}
\include{Ccl/concl}

% ==================================================================
% ANNEXES
\let\cleardoublepage\clearpage
{
\hypersetup{linkcolor=black}
\appendix
\renewcommand*{\toclevel@section}{0}
\include{Annexes/annexe}
}
 
% ==================================================================
% BIBLIOGRAPHIE
\makeatletter
\renewcommand*{\toclevel@chapter}{-1}
\backmatter
\bibliographystyle{apalike}
\bibliography{Manuscrit}
 
% ==================================================================
% COLOPHON
%\colophon{Ce document a été préparé à l'aide de l'éditeur de texte GNU
%  Emacs et du logiciel de composition typographique \LaTeXe.}


\end{document}
%%% Local Variables:
%%% mode: latex
%%% TeX-master: t
%%% End:
